...
let midia = [
  ...
  },
  {
    ...
    duracao: "1 temporada",
    sinopse:
      "Ao chegar a um vilarejo mexicano, um menino faz uma amizade improvável com um garoto local, e um segredo sombrio sela esse laço para sempre.",
    percentualGostei: "60", // mantendo conforme orientação
    percentualNaoGostei: "40", // mantendo conforme orientação
...
function aoPesquisarPorTitulo(titulo) {
  //#region Filmes
  let resultadosFilmes = midia.filter(
    (m) =>
      m.tipo == "filme" && m.nome.toLowerCase().includes(titulo.toLowerCase())
  );
  ...
  //#region Series
  let resultadosSeries = midia.filter(
    (m) =>
      m.tipo == "serie" && m.nome.toLowerCase().includes(titulo.toLowerCase())
  );
